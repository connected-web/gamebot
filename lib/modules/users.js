...
function updateUserIndex(api, data) {
  ...
  return api.bot.getUsers().then((users) => {
    ...
    api.findUserByName = (name) => {
      ...
      return Object.keys(index).filter((userId) => {
        const user = index[userId];
        return (poster.name + '').toLowerCase() === input ||
          (poster.profile &&
            (poster.profile.first_name + '').toLowerCase() === input ||
            (poster.profile.real_name_normalized + '').toLowerCase() === input)
      })[0];
    };
  });
}
