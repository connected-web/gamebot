...
function baron ({ api, model, targetPlayer, targetHand, currentPlayer, playerHand, playerCard, playedCard, targetCard }) {
  if (targetCard.value > playerCard.value) {
    ...
    api.respond(targetPlayer, `${api.getUserName(currentPlayer)} has targetted you with a ${playedCard.name}. Your ${displayCard(targetCard)} was of higher value than ${api.getUserName(currentPlayer)}'s ${displayCard(playedCard)}. ${api.getUserName(currentPlayer)} has been eliminated from the round.`)
