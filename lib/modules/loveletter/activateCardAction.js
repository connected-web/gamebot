...
function baron ({ api, model, targetPlayer, targetHand, currentPlayer, playerCard, playedCard, targetCard }) {
  if (targetCard.value > playerCard.value) {
    ...
    api.respond(targetPlayer, `${api.getUserName(currentPlayer)} has targetted you with a ${playedCard.name}. Your ${displayCard(targetCard)} was of higher value then ${api.getUserName(currentPlayer)}'s ${displayCard(playedCard)}. ${api.getUserName(currentPlayer)} has been eliminated from the round.`)
    api.respond(currentPlayer, `${api.getUserName(targetPlayer)} has a higher value card then you, a ${displayCard(targetCard)}, and you have been eliminated from the round.`)
    ...
    api.respond(targetPlayer, `${api.getUserName(currentPlayer)} has targetted you with a ${playedCard.name}. Your ${displayCard(targetCard)} was of lower value then ${api.getUserName(currentPlayer)}'s ${displayCard(playerCard)}. You have been eliminated from the round.`)
    api.respond(currentPlayer, `${api.getUserName(targetPlayer)} has a higher value card then you, a ${displayCard(targetCard)}, and you have been eliminated from the round.`)
  } else {
    ...
    api.respond(targetPlayer, `${api.getUserName(currentPlayer)} has targetted you with a ${playedCard.name}. Your ${displayCard(targetCard)} has the same value as ${api.getUserName(currentPlayer)}'s ${displayCard(playerCard)}. Neither of you have been eliminated from the round.`)
...
function handmaid ({ api, model, targetPlayer, targetCard, currentPlayer, playedCard }) {
  api.respond(model.gameChannel, `${api.getUserName(currentPlayer)} has played a ${displayCard(playedCard)} and is now protected until the start of their next turn.`)
...
function prince ({ api, model, targetPlayer, targetHand, targetCard, currentPlayer, playedCard }) {
  ...
  if (targetPlayer === currentPlayer) {
    ...
    api.respond(currentPlayer, `You have targetted yourself with a ${displayCard(playedCard)}, and have discarded your ${displayCard(targetCard)}, and have now drawn a ${newCard.name} (${newCard.value}).`)
    ...
    api.respond(targetPlayer, `You have been targetted by ${api.getUserName(currentPlayer)} with a ${displayCard(playedCard)}, and have discarded your ${displayCard(targetCard)}, and have now drawn a ${newCard.name} (${newCard.value}).`)
...
function countess ({ api, model, targetPlayer, currentPlayer, playedCard }) {
  api.respond(model.gameChannel, `${api.getUserName(currentPlayer)} has played a ${displayCard(playedCard)}.`)
