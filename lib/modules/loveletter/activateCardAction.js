...
function activateCardAction (api, model, targetPlayer, guessedCard) {
  ...
  if (typeof action === 'function') {
    ...
    api.respond(model.gameChannel, `${playedCard.action} is not a valid card action for this game.`)
    return
  }
...
function baron ({ api, model, targetPlayer, targetHand, currentPlayer, playerHand, playerCard, playedCard, targetCard }) {
  if (targetCard.value > playerCard.value) {
    ...
    api.respond(currentPlayer, `${api.getUserName(targetPlayer)} has a lower value card then you, a ${displayCard(targetCard)}, and they have been eliminated from the round.`)
    eliminate({ api, model, targetHand })
  } else {
...
function eliminate ({ api, model, playerHand, targetHand }) {
  ...
  if (discardedCard) {
    api.respond(model.gameChannel, `${api.getUserName(playerHand.player)} has been forced to discard a ${displayCard(discardedCard)}.`)
    playerHand.discardedCards.push(discardedCard.action)
  }
}
