...
function guard (api, model, targetPlayer, guessedCard) {
  ...
  if (targetHand.cards.includes(guessedCard.action)) {
    api.respond(model.gameChannel, `${api.mention(targetPlayer)} has been eliminated from the round. ${api.getUserName(currentPlayerId)} correctly guessed ${guessedCard.name} (${guessedCard.value}).`)
    api.respond(targetPlayer, `You have been eliminated from the round because ${api.getUserName(currentPlayerId)} correctly guessed that you had ${guessedCard.name} (${guessedCard.value}).`)
    api.respond(currentPlayerId, `You have successfully eliminated ${api.getUserName(targetPlayer)} by correctly guessing that they had ${guessedCard.name} (${guessedCard.value}).`)
  } else {
    api.respond(model.gameChannel, `${api.mention(targetPlayer)} has been targetted by ${api.getUserName(currentPlayerId)}, but they do not have ${guessedCard.name} (${guessedCard.value}).`)
    api.respond(targetPlayer, `You were targetted by ${api.getUserName(currentPlayerId)}, but they incorrectly guessed that you have ${guessedCard.name} (${guessedCard.value}).`)
    api.respond(currentPlayerId, `You targetted by ${api.getUserName(targetPlayer)}, but you incorrectly guessed that they had ${guessedCard.name} (${guessedCard.value}).`)
  }
}
...
function priest (api, model, targetPlayer, guessedCard) {
  ...
  api.respond(model.gameChannel, `${api.mention(targetPlayer)} has been targetted by ${api.getUserName(currentPlayerId)}; ${api.getUserName(targetPlayer)}'s role has been revealed to ${api.getUserName(currentPlayerId)}.'`)
