...
function baron ({ api, model, targetPlayer, targetHand, currentPlayer, playerHand, playerCard, playedCard, targetCard }) {
  if (targetCard.value > playerCard.value) {
    api.respond(model.gameChannel, `${api.getUserName(targetPlayer)} has been targetted by ${api.getUserName(currentPlayer)} with a ${displayCard(playedCard)}. ${api.getUserName(currentPlayer)} has been eliminated from the round, discarding a ${displayCard(playerCard)}.`)
