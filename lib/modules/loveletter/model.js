const statePath = 'state/loveletter-state.json'
...
function readModel (model, api) {
  return api.filesystem.read(statePath)
    ...
    .catch((ex) => {
      console.error('[Loveletter Model] Unable to read loveletter state', ex)
    })
    .then(() => model)
}
...
function create (api) {
  ...
  return readModel(model, api)
    .then((model) => {
      ...
      model.save = () => saveModel(model, api)
      model.read = () => readModel(model, api)
      return model
    })
}
