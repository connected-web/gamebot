...
function target (api, data, model) {
  ...
  const validTarget = validTargets.filter((target) => target.player === targetPlayer.id)[0]

  if (!validTarget) {
    const validNames = validTargets.map(target => api.getUserName(target.player))
    const validDisplayTargets = validNames.map(name => `*target _${name}_*`)
    api.respond(data, `_${searchTarget}_ is not a valid target. Please choose a valid player to target by responding with ${grammarList(validDisplayTargets, 'or')}.`)
    return
  }
  ...
  const playerHand = model.getPlayerHandFor(currentPlayerId)
