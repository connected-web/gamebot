function join (api, data, model) {
  ...
  api.respond(data.user, `Hey ${posterName}, you've joined love letter. You will be dealt your starting card after the game is started.`)
  api.respond(model.gameChannel, [`${posterName} has joined the game. Current players:`].concat(playerNames.join(', ')).join(' '))
}

module.exports = {
  name: 'Join game',
  regex: /^(join|enter) (game|love\s?letter|castle)/i,
