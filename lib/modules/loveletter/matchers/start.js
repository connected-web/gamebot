...
function start (api, data, model) {
  ...
  model.playerHands.forEach((playerHand) => {
    ...
    if (playerHand.player === model.currentPlayer()) {
      let secondCardId = playerHand.cards[1]
      let secondCard = cards.index[secondCardId]
      api.respond(playerHand.player, [
        `Hey ${playerName}! Your starting card is ${card.name} (${card.value})`,
        ` and your second card is ${secondCard.name} (${secondCard.value}).`,
        ` Please take your turn by responding with *play ${card.name}* or *play ${secondCard.name}*.`
      ].join(''))
    } else {
      api.respond(playerHand.player, `Hey ${playerName}! Your starting card is ${card.name} (${card.value}).`)
    }
  })
}
