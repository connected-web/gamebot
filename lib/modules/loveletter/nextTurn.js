...
function displayFinalCards ({ api, model, scoringPlayers }) {
  ...
  api.respond(model.gameChannel, `The final cards for each player are: ${grammarList(scoringPlayers.map(displayFinalCard))}`)
}
...
function roundOver ({ api, model }) {
  ...
  api.respond(model.gameChannel, `When you're ready, respond with *start next round* to continue play, or *stop game* to end.`)
}
