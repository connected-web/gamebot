...
function start(api, data) {
  ...
  state.playerOrder = assignPlayerOrder(state.players);
  state.turnCounter = 0;
...
function assignRoles(players) {
  var playerRoles, playerSeed, roleSeed;
  playerSeed = state.playerSeed;
  roleSeed = state.roleSeed;
...
function assignPlayerOrder(players) {
  var playerSeed = state.playerSeed;
  var playerOrder = [];
  var remainingPlayers = [].concat(players);
  for (var i = 0; i < players.length; i++) {
    var player = remainingPlayers[(playerSeed + i) % remainingPlayers.length];
...
module.exports = function (api, productionMode = true) {
  ...
  if (productionMode) {
    ...
    resistance.chooseSeeds = chooseSeeds;
