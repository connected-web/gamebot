...
function assignRoles(players, playerSeed, roleSeed) {
  ...
  if (configuration) {
    state.playerOrder = [];
    var remainingPlayers = [].concat(state.players);
    playerRoles = {};
...
function vote(api, data) {
  ...
  if (missingVotes > 0) {
    ...
    if (votesFor.length > votesAgainst.length) {
      ...
      api.respond('resistance', `Players ${grammarList(playerNameList)} have been assigned to the current mission; awaiting their responses.`);
    } else {
