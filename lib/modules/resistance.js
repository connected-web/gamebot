...
function getCurrentMission() {
  ...
  const missionConfiguration = state.customGame || roleConfigurations[players.length] || {
    missions: []
  };
...
function pick(api, data) {
  ...
  if (unmatched.length > 0) {
    ...
    api.respond(gameChannel, `${playerName} has picked ${grammarList(pickedNames)} to go on the next mission.`);
