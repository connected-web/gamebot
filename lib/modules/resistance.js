...
const matchers = {
  join: {
    regex: /^(join (the )?resistance|join spies)/i,
  ...
  start: {
    regex: /^(start resistance|resistance start|start game)$/i,
    channels: ['private', gameChannel],
    handler: start,
    description: 'Start a game of resistance with the registered players',
    examples: ['start resistance', 'resistance start', 'start game']
  },
  stop: {
...
function start(api, data) {
  ...
  if (state.roles) {
    ...
    api.respond('resistance', [
      ...
      `Player order is: ${grammarList(playerOrderNames, 'then', false)}. ${leaderName} is the first leader; pick a team using *pick Name1, Name 2, ...*`
    ].join(NL));
...
function vote(api, data) {
  ...
  if (missingVotes > 0) {
    ...
    if (votesFor.length > votesAgainst.length) {
      ...
      responses.push(`The leader token moves to ${api.getUserName(getLeader())}. Pick a team using *pick Name1, Name2, ...*`);
    }
  }
...
function checkForCompleteMission(api, data) {
  ...
  responses.push(`Mission Progress: ${getMissionProgress().join(' ')}`);
  const leader = getLeader();
  responses.push(`The leader token moves to ${api.getUserName(leader)}. Pick a team using *pick Name1, Name2, ...*`);
