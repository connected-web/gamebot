...
function pick(api, data) {
  ...
  if (unmatched.length > 0) {
    ...
    state.picks = [];
    api.respond('resistance', `Need to pick ${mission.requiredPlayers} valid players onto this mission.`);
...
function vote(api, data) {
  ...
  if (missingVotes > 0) {
    ...
    if (votesFor.length > votesAgainst.length) {
      ...
      if (checkForEndGame() === false) {
        // Move leader token onwards
        const mission = getCurrentMission();
        const twoFailsRequired = mission.twoFailsRequired ? ' (Two Fails Required)' : '';
        responses.push(`The leader token moves to ${api.getUserName(getLeader())}. Mission ${mission.counter} requires ${mission.requiredPlayers} players${twoFailsRequired}. Pick a team using *pick Name1, Name2, ...*`);
      }
    }
  }
...
function checkForEndGame() {
  ...
  state.missionHistory.forEach((result) => {
    if (result.name === team.Resistance.name) {
      results.resistance = results.resistance + 1;
    } else if (result.name === team.Spies.name) {
...
function assassinate(api, data) {
  ...
  if (playerRole && playerRole.assassin) {
    ...
    if (targetRole) {
      ...
      api.respond(data.user, `Could not assassinate ${targetName}, they do not have a role in this game.`);
    }
  } else {
