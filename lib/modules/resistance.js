...
function checkForCompleteMission(api, data) {
  ...
  responses.push(`Mission Progress: ${getMissionProgress().join(' ')}`);
  const leader = getLeader();
  responses.push(`The leader token moves to ${api.getUserName(leader)}. Mission ${nextMission.counter} requires ${nextMission.requiredPlayers} players. Pick a team using *pick Name1, Name2, ...*`);
...
function calculateMissionSuccess(playResults, twoFailsRequired = false) {
  ...
  if (twoFailsRequired && playerResults.fail > 1) {
    success = false
