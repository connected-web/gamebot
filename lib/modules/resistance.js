...
const role = {
  ...
  Assassin: {
    name: 'Assassin',
    team: team.Spies,
    description: 'May only play :success: or :fail:; has one chance to identify assasinate the Commander if the Resistance succeeds three missions. Attempt to fail three missions for your team.',
    knownToCommander: true,
    assassin: true
  },
  HiddenReverser: {
...
function start(api, data) {
  ...
  if (state.roles) {
    ...
    api.respond('resistance', `${playerName} tried to start the game, but there is no configuration for ${resistance.players.length} players.`);
  }
...
function assignRoles(players) {
  const playerRoles = {};
  const configuration = roles[players.length];
