...
function revealLocation (api, data, word, model) {
  ...
  if (!clue || clue.team !== teamName) {
    ...
    // remaining guesses
    let remainingGuesses = 1 + clue.number - clue.guesses.length
    if (location.team === 'assassin') {
      api.respond(model.gameChannel, `${word} is the :${location.team}: ${location.team} location, the :${teamName.toLowerCase()}: ${teamName} Team loses the game!`)
      model.gameOver = true
    } else if (location.team === 'neutral') {
      api.respond(model.gameChannel, `${word} is a :${location.team}: ${location.team} location, the :${teamName.toLowerCase()}: ${teamName} Team have ended their turn!`)
      nextTurn(api, data, model)
    } else if (location.team === activeTeam.name.toLowerCase()) {
      if (remainingGuesses > 0) {
        api.respond(model.gameChannel, `${word} is a :${location.team}: ${location.team} location. :${teamName.toLowerCase()}: ${teamName} Team have ${remainingGuesses} more guesses this turn!`)
      } else {
        api.respond(model.gameChannel, `${word} is a :${location.team}: ${location.team} location. :${teamName.toLowerCase()}: ${teamName} Team have ${remainingGuesses} remaining gsuesses, their turn has ended!`)
        nextTurn(api, data, model)
      }
    } else {
      api.respond(model.gameChannel, `${word} is a :${location.team}: ${location.team} location, the :${teamName.toLowerCase()}: ${teamName} Team have ended their turn!`)
