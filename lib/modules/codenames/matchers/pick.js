    ...
    if (location.team === 'assassin') {
      ...
      if (remainingWords === 0) {
        ...
        if (remainingGuesses === 1) {
          api.respond(model.gameChannel, `*${word}* is a :${location.team}: ${location.team} team location. :${teamName.toLowerCase()}: ${teamName} Team have 1 more guess this turn, alternatively use *end turn* to pass play to the other team.`)
        } else {
        ...
        api.respond(model.gameChannel, `*${word}* is a :${location.team}: ${location.team} team location. :${teamName.toLowerCase()}: ${teamName} Team have 0 remaining guesses, their turn has ended!`)
        nextTurn(api, data, model)
      }
    } else {
      api.respond(model.gameChannel, `*${word}* is a :${location.team}: ${location.team} team location, the :${teamName.toLowerCase()}: ${teamName} Team have ended their turn!`)
