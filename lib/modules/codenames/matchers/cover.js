...
function cover (api, data, model) {
  ...
  } else if (isValidRedLocation(model, word)) {
...
function isValidRedLocation (model, word) {
  return (model.words.filter((item) => item.word === word && item.team === 'red').length > 0)
}
...
function coverLocation (api, data, word, model) {
  ...
  if (location.revealed) {
    ...
    api.respond(model.gameChannel, `${word} has been covered as a ${model.getTeamIcon(location.team)} ${location.team} location, the :${teamName.toLowerCase()}: ${teamName} Team have ended their turn!`)
