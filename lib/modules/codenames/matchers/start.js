...
function start (api, data, model) {
  ...
  if (model.players.length === 2 || model.players.length === 3) {
    ...
    let team1Players = pickXFrom(Math.ceil(playerPool.length / 2), playerPool)
    let team2Players = playerPool
    ...
    if (Math.random() >= 0.5) {
      model.teams[0].teamId = 'team1'
      model.teams[1].teamId = 'team2'
    } else {
      model.teams[1].teamId = 'team1'
      model.teams[0].teamId = 'team2'
    }
  }
  model.started = true
  model.save()
