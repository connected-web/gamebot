...
function checkIfReady (api, data, model) {
  ...
  if (model.words.length > 0) {
    ...
    model.turnCounter = 0
    model.updateActiveSpymaster()
    let playerName = api.getUserName(model.activeSpymaster)
    api.respond(model.activeSpymaster, `${playerName} your operatives are now active. Give a one word clue, followed by a number; for example: *flying 3* to match 3 words relating to flying. Say *help clues* for more information.`)
  }
}
