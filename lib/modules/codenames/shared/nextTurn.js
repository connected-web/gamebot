...
function nextTurn (api, data, model) {
  ...
  if (activeTeam.solitaire) {
    api.respond(model.gameChannel, [
      ...
      `@${api.getUserById(solitaireSpyMaster).name} you need to *cover* a :${teamName.toLowerCase()}: ${teamName} Team word using the *cover _word_* command.`
    ].join(NL))
  } else {
    api.respond(model.gameChannel, [
      `It is now ${teamIcon} ${teamName} Team's turn!`,
      `@${api.getUserById(activeSpymaster).name} it is your turn to give a *clue*.`
    ].join(NL))
  }
}
