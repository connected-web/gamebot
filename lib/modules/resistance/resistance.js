...
const statePath = 'state/resistance-state.json'
const state = {}
clearState()

// Persist data between server restarts
function saveModel (model, api) {
  return api.filesystem.write(statePath, model)
}

function readModel (model, api) {
  return api.filesystem.read(statePath)
    .then((data) => {
    ...
    .catch((ex) => {
      console.error('[Resistance Model] Unable to read resistance state', ex)
    })
    .then(() => model)
}

function saveRecord () {
  /* Needs replacing with a stats call:
